...
#define BUZZER_A 10
#define BUZZER_B 21
#define delay 400  // Velocidade inicial da cobra
#define RED 200,0,0
...
int generateApple() {
    ...
    if (snake_length >= LED_COUNT) {
        return -1; // Indicar que ja não há mais espaço pra maçã
    }
...
void drawSnake() {
    npClearAllLEDs();
    if (snake_length > 0) {
        npSetLEDColor(snake[0], BLUE); // Cabeça azul
    }
    for (int i = 1; i < snake_length; i++) {
        npSetLEDColor(snake[i], RED); // Corpo vermelho
    }
    if (apple_position != -1) {
        npSetLEDColor(apple_position, GREEN);
    }
    npUpdateLEDs();
}
...
int main() {
    ...
    while (true) { // Loop infinito prareiniciar o jogo
        apple_position = generateApple();
        snake_length = 1;
        snake_direction = 1;
        game_over = false;
